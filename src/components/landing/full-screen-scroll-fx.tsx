...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    const tempWordBucket = useRef<Map<number, HTMLSpanElement[]>>(new Map());
    const splitWords = (text: string, sIdx: number) => {
      const words = text.split(/\s+/).filter(Boolean);
      if (!tempWordBucket.current.has(sIdx)) tempWordBucket.current.set(sIdx, []);
      const bucket = tempWordBucket.current.get(sIdx)!;
      return words.map((w, i) => (
        <span className="fx-word-mask" key={i}>
          <span
            className="fx-word"
            ref={(el) => {
              if (el) bucket.push(el);
            }}
          >
            {w}
          </span>
    ...
    const WordsCollector = ({ sIdx }: { sIdx: number }) => {
      useEffect(() => {
        const bucket = tempWordBucket.current.get(sIdx);
        if (bucket?.length) wordRefs.current[sIdx] = [...bucket];
      }, [sIdx]);
      return null;
    };
    ...
    const measureAndCenterLists = (toIndex = index, animate = true) => {
      const centerTrack = (container: HTMLDivElement | null, items: HTMLDivElement[]) => {
        ...
        if (animate) {
          gsap.to(container, { y: targetY, duration: (durations.change ?? 0.7) * 0.9, ease: "power3.out" });
        } else {
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-text" as string]: colors.text ?? "rgba(255,255,255,0.95)",
      ["--fx-overlay" as string]: colors.overlay ?? "rgba(0,0,0,0.4)",
      ["--fx-page-bg" as string]: colors.pageBg ?? "#fff",
      ["--fx-stage-bg" as string]: colors.stageBg ?? "#0f0a1e",
    ...
    return (
      ...
      >
        ...
        <div className="fx-scroll">
          <div className="fx-fixed-section" ref={fixedSectionRef} style={{ height: `${Math.max(1, total + 1) * 100}vh` }}>
            <div className="fx-fixed" ref={fixedRef}>
              <div className="fx-bgs" aria-hidden="true">
                {sections.map((s, i) => (
                  <div className="fx-bg" key={s.id ?? i}>
                    {s.renderBackground ? (
                      <div ref={(el) => { if (el) bgRefs.current[i] = el; }} className="fx-bg-inner fx-bg-custom">
                        {s.renderBackground(index === i, lastIndexRef.current === i)}
                      </div>
                    ) : (
                      <>
                        <div ref={(el) => { if (el) bgRefs.current[i] = el; }} className="fx-bg-inner" style={{ background: s.background }} />
          ...
          <div className="fx-end">
            {endContent ?? <p className="fx-fin">Scroll to explore</p>}
          </div>
        </div>
      </div>
    );
