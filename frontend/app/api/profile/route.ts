...
export async function PUT(request: Request) {
  try {
    ...
    if (quiz_notification_email !== undefined) {
      const raw =
        quiz_notification_email === null || quiz_notification_email === ''
          ? null
          : String(quiz_notification_email).trim()
      if (raw && (!raw.includes('@') || raw.length < 4)) {
        return NextResponse.json({ error: 'Invalid email for quiz results' }, { status: 400 })
      }
      updateData.quiz_notification_email = raw
    }
